...
public class Converter {
    ...
    public static Map<String, Object> toLua(VItem item){
    ...
    public static Object parse(IDEntry entry){
        return entry.get();
    }

    public static  Object parse(BirthEntry entry){
        return entry.get();
    }

    public static  Object parse(ClusterEntry entry){
        return entry.get().ids.toArray();
    }

    public static  Object parse(PlayerSetEntry entry){
        return entry.get().raw.stream().map(VOwnerData::playerName).toArray();
    }

    public static  Object parse(SlugEntry entry){
        return entry.get();
    }

    public static  Object parse(VTagEntry entry){
